...
/**
 * Tests the <code>XMLSerializer</code>
 */
public class XMLSerializerTest extends TestCase {
	...
	public void testRoot() throws Exception {
		...
		expect(config.handle(root, ctx)).andReturn("<listing/>");
		replay(config);
	...
	public void testValueWhenNullAndNotWrite() {
		...
		expect(config.getName()).andReturn("name");
	...
	public void testValueWhenNullAndWriteButNameNull() {
		ValueConfig config = createMock(ValueConfig.class);
		expect(config.getName()).andReturn(null);
	...
	public void testValueWhenEmptyAndWrite() {
		...
		expect(config.isWriteEmpty()).andReturn(true);
		replay(config);
		String result = serializer.serializeValue(config, "  ", ctx);
		verify(config);
	...
	public void testValueWhenNameBlank() {
		...
		expect(config.getName()).andReturn(null);
		replay(config);
		String result = serializer.serializeValue(config, "John Doé", ctx);
		verify(config);
	...
	public void testComplexWhenNullAndNotWrite() throws Exception {
		...
		expect(config.isWriteEmpty()).andReturn(false);
		replay(config);
		String result = serializer.serializeComplex(config, null, null, null, ctx.appendSegment("listing"));
		verify(config);
		assertEquals(StringUtils.EMPTY, result);
	}
	...
	public void testComplexWhenNoProperty() throws Exception {
		...
		expect(config.getName()).andReturn("listing");
		replay(config);
		String result = serializer.serializeComplex(config, null, null, new Object(), ctx.appendSegment("listing"));
		verify(config);
		assertEquals("<listing/>", result);
	}
	...
	public void testComplexWithOnlySubElements() throws Exception {
		...
		expect(config.getName()).andReturn("listing");
		expect(attributeConfig.handle(listing, ctx.appendSegment("listing"))).andReturn("");
		expect(elementConfig.handle(listing, ctx.appendSegment("listing"))).andReturn("<name>LocalMatters</name>");
	...
	public void testComplexWithOnlyAttributes() throws Exception {
		...
		expect(config.getName()).andReturn("listing");
		expect(attributeConfig.handle(listing, ctx.appendSegment("listing"))).andReturn(" id=\"123456\"");
		expect(elementConfig.handle(listing, ctx.appendSegment("listing"))).andReturn("");
	...
	public void testAttributeWhenNullAndNotWrite() {
		...
		expect(config.isWriteEmpty()).andReturn(false);
		replay(config);
		String result = serializer.serializeAttribute(config, null, ctx);
		verify(config);
		assertEquals(StringUtils.EMPTY, result);
	}
	...
	public void testAttribute() {
		...
		expect(config.getName()).andReturn("name");
		replay(config);
		String result = serializer.serializeAttribute(config, "John Doé", ctx);
		verify(config);
	...
	public void testListWhenContainsOnlyNullElements() throws Exception {
		...
		expect(config.isWriteEmpty()).andReturn(true);
		expect(config.getName()).andReturn("sports");
	...
	public void testList() throws Exception {
		...
		expect(elementConfig.handle("baseball", ctx.appendSegment("sports[0]"))).andReturn("<sport>baseball</sport>");
		expect(elementConfig.handle("hockey", ctx.appendSegment("sports[1]"))).andReturn("<sport>hockey</sport>");
		expect(config.getName()).andReturn("sports");
	...
	public void testMapWhenEmptyAndWrite() throws Exception {
		Config config = createMock(MapConfig.class);
		expect(config.isWriteEmpty()).andReturn(true);
		expect(config.getName()).andReturn("leisure");
		replay(config);
		String result = serializer.serializeMap(config, null, null, null, ctx);
		verify(config);
